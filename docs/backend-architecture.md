...
### 10.2 Durable notifications

Notifications are durable records, not inline third-party calls. `NotificationService.send_email_for_account` writes a `queued_notifications` row and picks a delivery mode by priority. `immediate` (OTP, password reset) sends synchronously in the request, then returns. `normal` writes the record `pending` and returns fast; `NotificationDrainJob` (`modules/notification/jobs/`, `cron_schedule = "*/5 * * * *"`) picks up pending, retry-due records ordered by priority then `next_retry_at` and sends each. The drain runs under its `JOB` actor, so every status transition it writes attributes to a concrete run.

A record carries `status` (`pending` → `processing` → `sent` → `delivered` | `failed` | `expired`), `retry_count` / `max_retries`, `next_retry_at`, and `expires_at`. A transient send failure increments `retry_count` and pushes `next_retry_at` out by exponential backoff, keeping the record retryable until `max_retries` is exceeded, at which point it is `failed`. A record past `expires_at` is marked `expired` rather than sent, and a TTL index on `expires_at` reclaims old rows. Delivery honors the account's notification preference unless the caller passes `bypass_preferences=True` (OTP and password reset do). The channel enum models `email` | `sms` | `push`; only email routes through this path today, with SMS and push as later slices on the same model.
