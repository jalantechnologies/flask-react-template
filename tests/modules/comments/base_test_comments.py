...
class BaseTestComments(unittest.TestCase):
    ACCESS_TOKEN_URL = "http://127.0.0.1:8080/api/access-tokens"
    HEADERS = {"Content-Type": "application/json"}

    DEFAULT_COMMENT_TEXT = "This is a test comment"
    DEFAULT_USERNAME = "aditi@aditi.io"
    DEFAULT_PASSWORD = "aditi_password"
    DEFAULT_FIRST_NAME = "Aditi"
    DEFAULT_LAST_NAME = "Kumari"
    DEFAULT_TASK_ID = "task012"
    ...
    def tearDown(self) -> None:
        try:
            CommentRepository.get_collection().delete_many({})
            AccountRepository.get_collection().delete_many({})
    ...
    def create_test_comment(self, account_id: str, task_id: str = None, text: str = None) -> Comment:
        return CommentService.create_comment(
            params=CreateCommentParams(
                account_id=account_id, task_id=task_id or self.DEFAULT_TASK_ID, text=text or self.DEFAULT_COMMENT_TEXT
            )
        )

    def create_multiple_test_comments(self, account_id: str, task_id: str = None, count: int = 3) -> list[Comment]:
        comments = []
        for i in range(count):
            comment = self.create_test_comment(
                account_id=account_id, task_id=task_id or self.DEFAULT_TASK_ID, text=f"Comment {i+1}"
            )
    ...
    def make_unauthenticated_comment_request(
        ...
        with app.test_client() as client:
            if method.upper() == "GET":
                ...
                return client.put(url, headers=self.HEADERS, data=json.dumps(data) if data is not None else None)
            elif method.upper() == "DELETE":
    ...
    def assert_comment_response(self, response_json: dict, expected_comment: Comment = None, **expected_fields):
        ...
        assert response_json.get("text") is not None
