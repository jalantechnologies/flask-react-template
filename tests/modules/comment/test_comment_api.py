...
class TestCommentApi(BaseTestComment):
    ...
    def test_create_comment_empty_body(self) -> None:
        ...
        # Send completely empty request (no data)
        ...
        # This should result in a 400 status but might not have JSON response
    ...
    def test_get_all_comments_with_comments(self) -> None:
        ...
        # Comments should be sorted by creation time (newest first)
        # Check that all comments are present (order may vary if created very quickly)
        comment_contents = [item["content"] for item in response.json["items"]]
        assert "Comment 1 content" in comment_contents
        assert "Comment 2 content" in comment_contents  
        assert "Comment 3 content" in comment_contents
    ...
    def test_comments_are_account_isolated_via_api(self) -> None:
        ...
        # Try to access account1's comment using account2's token
        ...
        # Verify comment is still accessible by the correct account
