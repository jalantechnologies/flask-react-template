...
class TestCommentApi(BaseTestComment):
    ...
    def test_create_comment_task_not_found(self) -> None:
        ...
        response = self.make_authenticated_comment_request("POST", account.id, non_existent_task_id, token, data=comment_data)

        self.assert_error_response(response, 404, CommentErrorCode.NOT_FOUND)
        assert "Task with id" in response.json.get("message")
    ...
    def test_get_specific_comment_success(self) -> None:
        ...
        response = self.make_authenticated_comment_request("GET", account.id, task.id, token, comment_id=comment.id)
    ...
    def test_get_specific_comment_not_found(self) -> None:
        ...
        self.assert_error_response(response, 404, CommentErrorCode.NOT_FOUND)
        assert "Comment with id" in response.json.get("message")
    ...
    def test_get_specific_comment_no_auth(self) -> None:
        ...
        response = self.make_unauthenticated_comment_request("GET", account.id, task.id, comment_id=comment.id)
    ...
    def test_update_comment_success(self) -> None:
        ...
        response = self.make_authenticated_comment_request("PATCH", account.id, task.id, token, comment.id, update_data)
    ...
    def test_update_comment_empty_body(self) -> None:
        ...
        response = self.make_authenticated_comment_request("PATCH", account.id, task.id, token, comment.id, data=None)
    ...
    def test_update_comment_not_found(self) -> None:
        ...
        response = self.make_authenticated_comment_request("PATCH", account.id, task.id, token, non_existent_comment_id, update_data)
    ...
    def test_update_comment_no_auth(self) -> None:
        ...
        response = self.make_unauthenticated_comment_request("PATCH", account.id, task.id, comment.id, update_data)
    ...
    def test_delete_comment_success(self) -> None:
        ...
        response = self.make_authenticated_comment_request("DELETE", account.id, task.id, token, comment.id)

        assert response.status_code == 204
        assert response.data == b""

        # Verify comment is soft deleted
        deleted_comment = CommentRepository.get_comment(comment_id=str(comment.id))
    ...
    def test_delete_comment_not_found(self) -> None:
        ...
        response = self.make_authenticated_comment_request("DELETE", account.id, task.id, token, non_existent_comment_id)
    ...
    def test_delete_comment_no_auth(self) -> None:
        ...
        response = self.make_unauthenticated_comment_request("DELETE", account.id, task.id, comment.id)
    ...
    def test_comments_are_task_isolated(self) -> None:
        ...
        self.create_test_comment(task_id=task1.id, account_id=account1.id, content="Task1 Comment")
        
        # Create comments for task2
        self.create_test_comment(task_id=task2.id, account_id=account2.id, content="Task2 Comment")
        
        # Get comments for task1
        response1 = self.make_authenticated_comment_request("GET", account1.id, task1.id, token1)
        assert response1.status_code == 200
        assert len(response1.json["items"]) == 1
        assert response1.json["items"][0]["content"] == "Task1 Comment"
        
        # Get comments for task2
        response2 = self.make_authenticated_comment_request("GET", account2.id, task2.id, token2)
        assert response2.status_code == 200
        assert len(response2.json["items"]) == 1
        assert response2.json["items"][0]["content"] == "Task2 Comment"
