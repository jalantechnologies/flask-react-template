...
class BaseTestComment(unittest.TestCase):
    ...
    DEFAULT_TASK_DESCRIPTION = "This is a test task"
    ...
    def get_comment_api_url(self, account_id: str) -> str:
        return f"http://127.0.0.1:8080/api/accounts/{account_id}/comments"
    ...
    def create_account_and_get_token(
        ...
        test_username = username or f"testuser_{str(id(self))}@example.com"
    ...
    def create_test_comment(self, account_id: str, task_id: str, content: Optional[str] = None) -> Comment:
    ...
    def make_authenticated_request(
        ...
        if comment_id:
            url = self.get_comment_by_id_api_url(account_id, comment_id)

            if method.upper() in ["GET", "DELETE"]:
                if query_params:
                    query_params += f"&task_id={task_id}"
                else:
                    query_params = f"task_id={task_id}"
        else:
            url = self.get_comment_api_url(account_id)
        ...
        if data is not None and method.upper() in ["POST", "PATCH"]:
    ...
    def make_unauthenticated_request(
        self, method: str, account_id: str, task_id: str, comment_id: Optional[str] = None, data: Optional[dict] = None
    ):
        if comment_id:
            url = self.get_comment_by_id_api_url(account_id, comment_id)
    ...
    def make_cross_account_request(
        ...
        if comment_id:
            url = self.get_comment_by_id_api_url(target_account_id, comment_id)
            if method.upper() in ["GET", "DELETE"]:
                url += f"?task_id={task_id}"
        else:
            url = self.get_comment_api_url(target_account_id)
            if method.upper() == "GET":
                url += f"?task_id={task_id}"
