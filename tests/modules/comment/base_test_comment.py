...
class BaseTestComment(unittest.TestCase):
    ...
    def assert_comment_response(self, response_json: dict, expected_comment: Comment = None, **expected_fields):
        ...
        if expected_comment:
            assert response_json["id"] == expected_comment.id
            assert response_json["task_id"] == expected_comment.task_id
            assert response_json["account_id"] == expected_comment.account_id
            assert response_json["content"] == expected_comment.content

        for field, expected_value in expected_fields.items():
            assert response_json[field] == expected_value, f"Expected {field} to be {expected_value}, got {response_json[field]}"
