...
class TestCommentService(BaseTestComment):
    ...
    def test_get_comment_for_nonexistent_task(self) -> None:
        non_existent_task_id = "507f1f77bcf86cd799439011"
    ...
    def test_get_paginated_comments_with_data(self) -> None:
        ...
        # Comments should be sorted by creation time (newest first by default)
        # Check that all comments are present (order may vary if created very quickly)
        comment_contents = [item.content for item in result.items]
    ...
    def test_delete_comment(self) -> None:
        ...
        # Verify comment can no longer be retrieved
        get_params = GetCommentParams(
            account_id=self.account.id,
            task_id=self.task.id,
            comment_id=created_comment.id
