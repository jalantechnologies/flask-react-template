...
from modules.task.types import CreateCommentParams, CreateTaskParams, Comment, Task
...
class BaseTestComment(unittest.TestCase):
    ...
    def make_unauthenticated_comment_request(self, method: str, account_id: str, task_id: str, comment_id: str = None, data: dict = None):
        if comment_id:
            url = self.get_comment_by_id_api_url(account_id, task_id, comment_id)
        else:
            url = self.get_comments_api_url(account_id, task_id)
