...
*   Temporal CLI - [Download](https://learn.temporal.io/getting_started/typescript/dev_environment/#set-up-a-local-temporal-service-for-development-with-temporal-cli)\
    *Note: Temporal CLI is only required if you intend to use distributed workflows. If you run `npm run serve -- --no-temporal`, you can skip installing Temporal CLI, but distributed workflow features will not be available.*
